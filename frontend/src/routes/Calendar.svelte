<script>
  ...
  // from the SQL MIN), so the global earliest may be newer than their own
  // data. Employees and assistants only see their own data, so the global
  // lower bound is a safe and useful cap for them.
