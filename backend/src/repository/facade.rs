...
use super::{
    ...
    NotificationDb, PayrollReportQueueDb, ReopenRequestDb, ReportDb, SessionDb, SettingsDb,
...
impl Db {
    pub fn new(pool: DatabasePool, broadcaster: NotificationBroadcaster) -> Self {
        Db {
            ...
            flextime_adjustments: FlextimeAdjustmentDb::new(pool.clone()),
            ...
            payroll_queue: PayrollReportQueueDb::new(pool.clone()),
            error_queue: ErrorNotificationQueueDb::new(pool.clone()),
            email_queue: EmailQueueDb::new(pool),
        }
    }
}
